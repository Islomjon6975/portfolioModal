...
window.addEventListener("DOMContentLoaded", () => {
  ...
  const slides = document.querySelectorAll(".offer__slide"),
    ...
    widthh = window.getComputedStyle(slidesWrapper).width,
  ...
  next.addEventListener("click", () => {
    if (offset == +widthh.slice(0, widthh.length - 2) * (slides.length - 1)) {
      offset = 0;
    } else {
      offset += +widthh.slice(0, widthh.length - 2);
    }
  ...
  prev.addEventListener("click", () => {
    if (offset == 0) {
      offset = +widthh.slice(0, widthh.length - 2) * (slides.length - 1);
    } else {
      offset -= +widthh.slice(0, widthh.length - 2);
    }
    slidesField.style.transform = `translateX(-${offset}px)`;
  ...
  dots.forEach((dot) => {
    dot.addEventListener("click", (e) => {
      ...
      offset = +widthh.slice(0, widthh.length - 2) * (slideTo - 1);
      slidesField.style.transform = `translateX(-${offset}px)`;
      if (slides.length < 10) {
        current.textContent = `0${slideIndex}`;
      } else {
